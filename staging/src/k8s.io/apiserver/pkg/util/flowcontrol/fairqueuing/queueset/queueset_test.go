...
func newObserverPair(clk clock.PassiveClock) metrics.RatioedObserverPair {
	rop, err := metrics.PriorityLevelConcurrencyObserverPairVec.WithLabelValues(1, 1, "test")
	if err != nil {
		panic(err)
	}
	return rop
...
func newExecSeatsObserver(clk clock.PassiveClock) metrics.RatioedObserver {
	ro, err := metrics.PriorityLevelExecutionSeatsObserverVec.WithLabelValues(1, "test")
	if err != nil {
		panic(err)
	}
	return ro
