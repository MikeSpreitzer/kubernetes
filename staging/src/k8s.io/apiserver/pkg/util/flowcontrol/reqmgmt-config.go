...
func (reqMgr *requestManager) digestConfigObjects(newPLs []*rmtypesv1a1.PriorityLevelConfiguration, newFSs []*rmtypesv1a1.FlowSchema) {
	...
	if !haveExemptPL {
		haveExemptPL = newRMState.imaginePL(0, reqMgr.requestWaitLimit, &shareSum)
	}
	if !haveCatchAllPL {
		haveCatchAllPL = newRMState.imaginePL(1, reqMgr.requestWaitLimit, &shareSum)
	}
	if !haveExemptFS {
...
func (newRMState *requestManagerState) imaginePL(protoIdx int, requestWaitLimit time.Duration, shareSum *float64) bool {
