...
func DefaultFlowSchemaObjects() []*rmv1a1.FlowSchema {
	verbAll := strings(rmv1a1.VerbAll)
	apiGroupAll := strings(rmv1a1.APIGroupAll)
	apiGroupCore := strings("")
	resourceAll := strings(rmv1a1.ResourceAll)
	ruleRscAll := rule(verbAll, apiGroupAll, resourceAll)
	ruleNonRscAll := rmv1a1.PolicyRule{
		Verbs:           verbAll,
		NonResourceURLs: strings(rmv1a1.NonResourceAll)}
	subjectsAll := groups("system:authenticated", "system:unauthenticated")
	return []*rmv1a1.FlowSchema{
		fs("system-top", 1500, "",
			rmv1a1.PolicyRuleWithSubjects{
				Subjects: groups("system:masters"),
				Rule:     ruleRscAll},
			rmv1a1.PolicyRuleWithSubjects{
				Subjects: groups("system:masters"),
				Rule:     ruleNonRscAll},
		...
		fs("system-high", 2500, rmv1a1.FlowDistinguisherMethodByUserType,
			rmv1a1.PolicyRuleWithSubjects{
				Subjects: groups("system:nodes"),
				Rule:     rule(verbAll, apiGroupCore, strings("nodes"))},
			rmv1a1.PolicyRuleWithSubjects{
				Subjects: groups("system:nodes"),
			...
			rmv1a1.PolicyRuleWithSubjects{
				Subjects: svcAccount("*"),
				// Namespace: "kube-system",
				Rule: rule(verbAll, apiGroupCore, strings("endpoints", "configmaps", "leases"))},
		),
