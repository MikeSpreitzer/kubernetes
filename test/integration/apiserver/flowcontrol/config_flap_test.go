...
// The test runs a regular server, usually producing new config.
// The test waits for the new config to be present.
// Then the test starts temporarily suppressing the regular config-producer
// and runs a producer of the old config.
// Then the test waits for the old config to be established.
// Then the test terminates the old-style producer and resumes the regular (new) config producer.
// Then the test waits for the new config to be established.
...
func flapTo(t *testing.T, ctx context.Context, trial string, v134 bool) bool {
	...
	client, _, tearDown := setupAnother(t, ctx, 100, 100, false, v134)
	defer func() {
