...
type configController struct {
	...
	// tracedReference holds the latest priority level reference from
	// a FlowSchema with the tracer name.
	tracedReference string

	// this condition is broadcast whenever tracedReference changes
	traceCond sync.Cond
...
func (meal *cfgMeal) digestFlowSchemasLocked(newFSs []*fctypesv1a1.FlowSchema) {
	...
	for i, fs := range newFSs {
		...
		if fs.Name == tracerName {
			meal.cfgCtl.setTracedReferenceLocked(fs.Spec.PriorityLevelConfiguration.Name)
		}
...
func (cfgCtl *configController) setTracedReferenceLocked(ref string) {
	klog.V(4).Infof("Setting traced reference to %q", ref)
	cfgCtl.tracedReference = ref
	cfgCtl.traceCond.Broadcast()
}

func (cfgCtl *configController) waitForTracedReference(ref string) {
	cfgCtl.lock.Lock()
	defer cfgCtl.lock.Unlock()
	for cfgCtl.tracedReference != ref {
		klog.V(4).Infof("Waiting for traced reference to become %q", ref)
		cfgCtl.traceCond.Wait()
	}
	klog.V(4).Infof("Traced reference became %q", ref)
}
