...
// rvector is an implementation of ResourceVersion
// that works for storage in independent shards.
// It handles adding shards naturally.
// It does not behave properly when a shard is retired,
// if a retired shard no longer appears in the
// representation.
...
func ParseRVector(rvS string) (ResourceVersion, error) {
	return parseRVector(rvS)
}

func parseRVector(rvS string) (rvector, error) {
	if rvS == UndefinedAsString {
		return rvector{}, nil
	}
	if rvS == AnyAsString {
		return rvector{vv: versionVector{anyEntry()}}, nil
	}
	ans := rvector{vv: make(versionVector, 0, len(rvS)/8)}
	for rem, remLen := rvS, len(rvS); remLen > 0; {
		...
		if bindIdx < 0 {
			return ans, fmt.Errorf("rvector %q has no equals in entry %q", rvS, entry)
		}
		version, err := strconv.ParseUint(entry[bindIdx+1:], 10, 64)
		if err != nil {
			return ans, fmt.Errorf("rvector %q has malformed version number in entry %q: %w", rvS, entry, err)
		}
...
func (rv rvector) IsDefined() bool {
	return len(rv.vv) > 0
}
...
func (rv rvector) IsAny() bool {
	return len(rv.vv) == 1 && rv.vv[0] == anyEntry()
}

// IsSpecial reveals whether the given rvector is one of the special
// values that can not come from a store.
func (rv rvector) IsSpecial() bool {
	return rv.IsAny() || !rv.IsDefined()
}

// String returns the canonical representation to appear in
// ObjectMeta.ResourceVersion
func (rv rvector) CanonicalString() string {
	return rv.String()
}

// String returns the canonical representation to appear in
// ObjectMeta.ResourceVersion
func (rv rvector) String() string {
	if !rv.IsDefined() {
		return ""
	}
	if rv.IsAny() {
...
func (rv rvector) StringForFilename() string {
	if !rv.IsDefined() {
		return "undefined"
	}
	return rv.String()
...
// Compare this with another given rvector.
// Unspecified and Any are incomparable with other values.
func (rv rvectorPair) Compare() Comparison {
	other := rv.other
	if !rv.IsDefined() {
		otherIsUndef := !other.IsDefined()
...
// Union combines this and the other rvector to produce
...
func (rv rvectorPair) Union() ResourceVersion {
	other := rv.other
	if rv.IsSpecial() {
		return other
	}
...
type rvectorPair struct {
	rvector
	other rvector
}

func (rv rvector) Parse(otherS string) (PairedResourceVersion, error) {
	other, err := parseRVector(otherS)
	if err != nil {
		return nil, err
	}
	return rvectorPair{rvector: rv, other: other}, nil
}
