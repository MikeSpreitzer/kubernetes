...
type TestableInterface interface {
	...
	// objects.  Returns a bool indicating whether updates were attempted,
	// another bool indicating whether a retry is needed to recover from
	// errors, and a Duration to wait before trying again if updates are
	// desired but suppressed (zero otherwise).
	SyncOne() (bool, bool, time.Duration)
}
