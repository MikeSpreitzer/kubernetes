...
func SetDefaults_PriorityLevelConfiguration(obj *v1alpha1.PriorityLevelConfiguration) {
	if !obj.Spec.Exempt {
		...
		if obj.Spec.Queues == 0 {
			obj.Spec.Queues = PriorityLevelConfigurationDefaultQueues
		}
		if obj.Spec.Queues == 0 {
			obj.Spec.QueueLengthLimit = PriorityLevelConfigurationDefaultQueueLengthLimit
		}
	}
}
