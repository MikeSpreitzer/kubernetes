	...
	for i := 1; i <= 3; i++ {
		...
		t.Run(fmt.Sprintf("trial%d:", i), func(t *testing.T) {
			...
			for j := 0; ; {
				...
				if true {
					cts.establishAPIObjects(trialStep, newGoodPLMap, newFSMap)
				} else {
...
func (cts *ctlTestState) establishAPIObjects(trialStep string, newPLs map[string]*fcv1a1.PriorityLevelConfiguration, newFSs map[string]*fcv1a1.FlowSchema) {
	...
	if cts.tracer == nil {
		cts.tracer, err = fsIfc.Create(&fcv1a1.FlowSchema{
			...
			Spec: fcv1a1.FlowSchemaSpec{
				PriorityLevelConfiguration: fcv1a1.PriorityLevelConfigurationReference{trialStep},
				MatchingPrecedence:         42}})
	} else {
		t2 := cts.tracer.DeepCopy()
		t2.Spec.PriorityLevelConfiguration.Name = trialStep
	...
	cts.cfgCtl.waitForTracedReference(trialStep)
}
