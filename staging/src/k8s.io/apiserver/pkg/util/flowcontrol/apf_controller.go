...
type flowSchemaState struct {
	// The last time this controller made an update.
	...
	// lastUpdateTime+minUpdateWait and alreadyAdjusted is false then
...
func (cfgCtl *configController) SyncOne() (bool, bool, bool, time.Duration) {
	...
	triedWrites, didWrites, err, wait := cfgCtl.digestConfigObjects(newPLs, newFSs)
	if err == nil {
...
func (cfgCtl *configController) digestConfigObjects(newPLs []*fctypesv1a1.PriorityLevelConfiguration, newFSs []*fctypesv1a1.FlowSchema) (bool, bool, error, time.Duration) {
	...
	if len(errs) == 0 {
		return triedWrites, didWrites, nil, wait
	}
	return triedWrites, didWrites, errorutils.NewAggregate(errs), wait
}
...
func (meal *cfgMeal) digestFlowSchemasLocked(newFSs []*fctypesv1a1.FlowSchema) {
	...
	for fsName, fss := range meal.cfgCtl.flowSchemaStates {
		if _, ok := fsMap[fsName]; (!ok) && fss.lastUpdateTime.Add(fss.updateWait+2*maxUpdateWait).Before(now) {
...
func (meal *cfgMeal) presyncFlowSchemaStatus(fs *fctypesv1a1.FlowSchema, isDangling bool, plName string) {
	...
	if fss == nil {
		...
		klog.V(5).Infof("%s at %s: initialized state of FlowSchema %s at ResourceVersion %s; updateWait=%s", cfgCtl.name, now.Format(timeFmt), fs.Name, fs.ResourceVersion, updateWait)
	} else {
		...
		} else if now.Before(fss.lastUpdateTime.Add(minUpdateWait)) {
		...
		} else if delta := now.Sub(nextUpdateTime); delta > 0 {
			oldWait := fss.updateWait
