...
func TestConfigConsumerFight(t *testing.T) {
	...
	for j := 0; j < 5; j++ {
		...
		wait := time.Hour
		foreach(func(invert bool, i int, ctl utilfc.TestableInterface) {
			triedUpdates, needRetry, thisWait := ctl.SyncOne()
			if needRetry {
				t.Errorf("Error for invert=%v, i=%d", invert, i)
			}
			t.Logf("For invert=%v, i=%d: triedUpdates=%v, wait=%s", invert, i, triedUpdates, thisWait)
			if thisWait > 0 {
				wait = utilfc.DurationMin(wait, thisWait)
			} else if triedUpdates {
				changeSum += 1
				wait = utilfc.DurationMin(wait, time.Millisecond*150)
			}
		})
		t.Logf("ChangeSum[size=%d, j=%d] at %s is %d", size, j, clk.Now().Format(timeFmt), changeSum)
