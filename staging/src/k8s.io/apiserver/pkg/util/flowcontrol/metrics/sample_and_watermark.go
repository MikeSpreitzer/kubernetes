...
// SampleAndWaterMarkPairVec makes pairs of RatioedObservers that
// track samples and watermarks.
type SampleAndWaterMarkPairVec struct {
	urVec SampleAndWaterMarkObserverVec
}

var _ RatioedObserverPairVec = SampleAndWaterMarkPairVec{}
...
func (spg SampleAndWaterMarkPairVec) WithLabelValues(initialWaitingDenominator, initialExecutingDenominator float64, labelValues ...string) (RatioedObserverPair, error) {
	return spg.WithLabelValuesSafe(initialWaitingDenominator, initialExecutingDenominator, labelValues...), nil
}

func (spg SampleAndWaterMarkPairVec) WithLabelValuesSafe(initialWaitingDenominator, initialExecutingDenominator float64, labelValues ...string) RatioedObserverPair {
	RequestsWaiting, _ := spg.urVec.WithLabelValues(initialWaitingDenominator, append([]string{labelValueWaiting}, labelValues...)...)
	RequestsExecuting, _ := spg.urVec.WithLabelValues(initialExecutingDenominator, append([]string{labelValueExecuting}, labelValues...)...)
...
// SampleAndWaterMarkObserverVec creates RatioedObservers that
...
var _ RatioedObserverVec = SampleAndWaterMarkObserverVec{}
...
// Generate makes a new RatioedObserver
func (swg *sampleAndWaterMarkObserverVec) WithLabelValues(initialDenominator float64, labelValues ...string) (RatioedObserver, error) {
	return swg.WithLabelValuesSafe(initialDenominator, labelValues...), nil
}

func (swg *sampleAndWaterMarkObserverVec) WithLabelValuesSafe(initialDenominator float64, labelValues ...string) RatioedObserver {
	ratio := swg.initialNumerator / initialDenominator
	when := swg.clock.Now()
	return &sampleAndWaterMarkHistograms{
		...
		sampleAndWaterMarkAccumulator: sampleAndWaterMarkAccumulator{
			...
			numerator:  swg.initialNumerator,
			ratio:      ratio,
			loRatio:    ratio,
			hiRatio:    ratio,
		}}
...
var _ RatioedObserver = (*sampleAndWaterMarkHistograms)(nil)
