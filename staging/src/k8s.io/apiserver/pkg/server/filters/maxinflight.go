...
type requestWatermark struct {
	phase         string
	observersFunc func() (readOnlyObserver, mutatingObserver fcmetrics.RatioedObserver)
...
func (w *requestWatermark) ensureInitialized(readonlyLimit, mutatingLimit int) {
	w.initializeOnce.Do(func() {
		w.readOnlyObserver, w.mutatingObserver = w.observersFunc()
		if readonlyLimit != 0 {
			w.readOnlyObserver.SetDenominator(float64(readonlyLimit))
		}
		if mutatingLimit != 0 {
			w.mutatingObserver.SetDenominator(float64(mutatingLimit))
		}
	})
}
